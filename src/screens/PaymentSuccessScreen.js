...
export default function PaymentSuccessScreen({ navigation, route }) {
  const {
    event = { title: 'John & Mary Wedding' },
    amount = 10000,
    paymentMethod = 'mtn',
    phoneNumber = '0781 234 567',
    total = 10000,
  } = route?.params || {};
  ...
  const handleShare = async () => {
    try {
      await Share.share({
        message: `I just contributed ${formatAmount(amount)} to ${event.title} via Contriba! 🎉`,
      });
    } catch (e) {
      console.log(e);
    }
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <Animated.View style={[styles.card, { opacity: fadeAnim }]}>
          ...
          <View style={styles.methodInfo}>
            <Text style={styles.methodName}>via {PAYMENT_LABELS[paymentMethod]}</Text>
            {paymentMethod !== 'visa' && (
              <Text style={styles.methodPhone}>{phoneNumber}</Text>
            )}
          </View>
        </Animated.View>
        ...
        <Animated.View style={[styles.receiptCard, { opacity: fadeAnim }]}>
          ...
          <View style={styles.receiptRow}>
            ...
            <Text style={styles.receiptValue}>RWF 0</Text>
          </View>
          <View style={styles.receiptDivider} />
          <View style={styles.receiptRow}>
            <Text style={styles.receiptTotalLabel}>Total</Text>
            <Text style={styles.receiptTotalValue}>{formatAmount(total)}</Text>
          </View>
        </Animated.View>
        ...
        <Animated.View style={[styles.receiptNote, { opacity: fadeAnim }]}>
          ...
          <Text style={styles.receiptNoteText}>A receipt has been sent to your phone.</Text>
        </Animated.View>
      ...
      <View style={styles.footer}>
        <TouchableOpacity style={styles.btnPrimary} onPress={handleBackToEvent} activeOpacity={0.85}>
          <Text style={styles.btnPrimaryText}>Back to Event</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.btnOutline} onPress={handleShare} activeOpacity={0.85}>
          <Text style={styles.btnOutlineText}>Share with others</Text>
          <Ionicons name="share-social-outline" size={18} color={WINE} style={{ marginLeft: 8 }} />
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  checkCircle: {
    width: 110, height: 110, borderRadius: 55, backgroundColor: GREEN,
    alignItems: 'center', justifyContent: 'center',
  ...
  card: {
    backgroundColor: LIGHT_GREY, borderRadius: 14, padding: 16,
  ...
  receiptNoteText: { fontSize: 13, color: DARK_GREY, flex: 1 },
  ...
  btnOutline: {
    borderWidth: 1.5, borderColor: WINE, borderRadius: 14, paddingVertical: 15,
    alignItems: 'center', justifyContent: 'center', flexDirection: 'row',
  },
  btnOutlineText: { color: WINE, fontSize: 16, fontWeight: '700' },
});